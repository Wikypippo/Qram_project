...
### Circuit definition e usage ####

# definition 1
'''
nqbits = 3
nbits = 3
circuit = QuantumCircuit(nqbits,nbits)
circuit.h([0,2])
circuit.ch(0,1)
...
circuit.initialize(initial_state, [0,1]) # Apply initialisation operation to the 0th qubit
circuit.measure_all()
draw_circuit(circuit)  # Let's view our circuit
res = run(circuit,1000)
counts(res)
...
circuit.barrier()
circuit = circuit.compose(qc,[3,0])
circuit.barrier()
draw_circuit(circuit)  # Let's view our circuit
'''
